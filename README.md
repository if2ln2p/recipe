...
목록 화면 오른쪽 위에서 두 가지를 고를 수 있습니다. 고른 값은 브라우저에 저장되어
다음에 열 때도 유지되며, 기기마다 따로 기억됩니다.

- **한 줄 `−` `n` `+`** — 한 줄에 표시할 레시피 개수 (1~4)
- **썸네일** — 목록 카드에 사진을 보일지 여부
- **태그** — 목록 카드에 태그를 보일지 여부

썸네일과 태그를 모두 끄면 이름과 인분만 있는 컴팩트한 목록이 됩니다.
둘 다 목록 카드에만 적용되며, 위쪽 태그 필터와 상세 페이지는 그대로입니다.
