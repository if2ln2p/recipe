...
목록 화면 오른쪽 위 `크기 · 작게 / 보통 / 크게`로 카드 크기를 고를 수 있습니다.
카드가 작을수록 한 줄에 더 많이 들어갑니다 (본문 760px 기준 4개 / 3개 / 2개).
고른 값은 브라우저에 저장되어 다음에 열 때도 유지되며, 기기마다 따로 기억됩니다.

크기 단계를 조정하려면 [assets/app.js](assets/app.js)의 `CARD_SIZES`(카드 최소 너비, px)를 고치세요.
한 줄 개수는 이 값에 맞춰 그리드가 자동으로 정합니다.
