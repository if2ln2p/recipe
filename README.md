...
목록 화면에서 네 가지를 고를 수 있습니다. 고른 값은 브라우저에 저장되어
다음에 열 때도 유지되며, 기기마다 따로 기억됩니다.

검색창 아래:

- **태그 검색** — 태그 필터 목록을 보일지 여부

결과 수 옆:

- **`−` `n` `+`** — 한 줄에 표시할 레시피 개수 (1~4)
- **썸네일** — 목록 카드에 사진을 보일지 여부
- **태그** — 목록 카드에 태그를 보일지 여부

썸네일과 태그를 모두 끄면 이름만 있는 컴팩트한 목록이 됩니다.
둘 다 목록 카드에만 적용되며 상세 페이지는 그대로입니다.

태그 검색을 숨겨도 이미 고른 태그는 계속 걸려 있습니다. 풀려면 다시 켜거나 `초기화`를 누르세요.

아직 고른 적이 없으면 화면 폭에 맞는 개수가 기본으로 잡힙니다 (폰 1개, 760px 본문 3개).
화면이 좁아 더 늘릴 수 없으면 `+`가 흐려지고, 고른 값보다 적게 보여줍니다.
이때 고른 값 자체는 남아 있어서 넓은 화면으로 돌아오면 그대로 복원됩니다.

관련 상수는 [assets/app.js](assets/app.js) 위쪽에 모여 있습니다.

- `MAX_COLUMNS` — 고를 수 있는 최대 개수
- `AUTO_CARD_W` — 기본 개수를 정할 때 기준으로 삼는 카드 폭
- `MIN_CARD_W` — 이보다 좁아지면 그 개수를 못 쓰게 막는 하한
